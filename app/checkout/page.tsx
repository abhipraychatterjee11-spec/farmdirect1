...
export default function Checkout() {
  ...
  function useCurrentLocation() {
    if (!navigator.geolocation) { setLocationMessage("We couldn't detect your location. Please try again or enter your address manually."); return; }
    setLocationMessage("Getting your location...");
    navigator.geolocation.getCurrentPosition(
      (position) => { setLocation({ latitude: position.coords.latitude, longitude: position.coords.longitude }); setLocationMessage("✓ Current location captured"); },
      (position) => setLocationMessage(position.code === position.PERMISSION_DENIED ? "Location permission was not granted. You can still enter your address manually." : "We couldn't detect your location. Please try again or enter your address manually."),
  ...
  async function placeOrder() {
    ...
    try {
      ...
      const result = await supabase.rpc("create_consumer_order", {
        ...
        p_delivery_latitude: location?.latitude ?? null,
        p_delivery_longitude: location?.longitude ?? null,
      });
      if (result.error) {
  ...
  return <main className="page-shell"><div className="mx-auto max-w-2xl p-8"><Link href="/cart">← Cart</Link><h1 className="mt-5 text-3xl font-extrabold">Confirm your order</h1><p className="mt-2 rounded-lg bg-amber-50 p-3 text-sm">Demo / Simulated Payment — no real payment will be collected.</p><p className="mt-5">{items.length} cart item(s). Final prices, stock, sellers and total are verified by PostgreSQL at order time.</p><label className="mt-5 block font-semibold">Delivery address<textarea value={address} onChange={(event) => setAddress(event.target.value)} className="mt-2 w-full rounded-lg border p-3"/></label><section className="mt-4 rounded-xl border border-green-950/10 p-4"><p className="font-semibold">Delivery location</p><button type="button" onClick={useCurrentLocation} className="mt-2 w-full rounded-xl border border-green-950/15 px-4 py-3 text-sm font-bold text-leaf">Use My Current Location</button>{locationMessage && <p className="mt-2 text-sm text-slate-600">{locationMessage}</p>}</section><button onClick={placeOrder} disabled={busy} className="mt-5 rounded-xl bg-leaf px-5 py-3 font-bold text-white disabled:opacity-60">{busy ? "Placing order…" : "Place simulated order"}</button>{message && <p className="mt-5 rounded-lg bg-green-50 p-3 text-sm">{message}</p>}</div></main>;
}
