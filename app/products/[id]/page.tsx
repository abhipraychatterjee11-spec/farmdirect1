...
export default function ProductDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="page-shell"><div className="mx-auto max-w-3xl p-8"><Link href="/marketplace" className="text-leaf">← Marketplace</Link><section className="mt-6 rounded-2xl bg-white p-7 shadow-sm"><p className="text-sm font-bold text-leaf">{product.category} · {product.status}</p><h1 className="mt-2 text-3xl font-extrabold">{product.name}</h1><p className="mt-3 text-slate-600">{product.description || "Fresh farm produce."}</p><p className="mt-5 text-xl font-bold">₹{product.price_inr} / {product.unit}</p><p className="mt-2">Available: <b>{product.available_quantity} {product.unit}</b></p><p className="mt-1 text-sm text-slate-500">Seller: {product.seller_name}</p><div className="mt-6 flex items-center gap-3"><button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="rounded border px-3 py-2">−</button><span>{quantity}</span><button onClick={() => setQuantity(Math.min(product.available_quantity, quantity + 1))} className="rounded border px-3 py-2">+</button><button onClick={addToCart} className="rounded-xl bg-leaf px-5 py-3 font-bold text-white">Add to cart</button></div>{message && <p className="mt-4 text-sm text-leaf">{message}</p>}</section></div></main>;
}
