...
export default function Orders() {
  ...
  const active = useMemo(() => orders.filter((order) => activeStatuses.has(order.status)), [orders]); const past = useMemo(() => orders.filter((order) => !activeStatuses.has(order.status) && order.status !== "cancelled"), [orders]); const cancelled = useMemo(() => orders.filter((order) => order.status === "cancelled"), [orders]);
